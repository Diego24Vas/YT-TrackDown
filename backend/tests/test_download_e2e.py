...
def test_download_video_process_e2e(client):
    # Short 1 second video: https://www.youtube.com/watch?v=tPEE9ZwTmy0
    payload = {
        "urls": ["https://www.youtube.com/watch?v=tPEE9ZwTmy0"],
        "format": "mp4",
    ...
    # Poll status until completed or timeout (20s max)
    start_time = time.time()
    completed = False
    while time.time() - start_time < 25:
        ...
        if data["status"] == DownloadStatus.COMPLETED.value:
            ...
            pytest.fail(f"Video download failed with error: {data.get('error_message')}")
        time.sleep(1)

    assert completed, "Video download did not complete in time"

    # Verify download file endpoint returns the MP4 video
    file_res = client.get(f"/api/downloads/{item_id}/file")
    assert file_res.status_code == 200
